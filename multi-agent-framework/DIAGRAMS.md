...
graph TB
    ...
    subgraph GatewayLayer["服务网关层 / Gateway Layer"]
        G1[请求路由]
        G2[认证鉴权]
        G3[限流熔断]
        G4[版本切流]
    ...
    subgraph SchedulerLayer["调度编排层 / Orchestration Layer"]
        S1[定时触发 Cron]
        S2[事件触发 Webhook]
        S3[API 触发]
        S4[手动触发]
    ...
    subgraph AgentLayer["多智能体协同层 / Multi-Agent Core"]
        P[规划智能体 Planner]
        E1[专家层 - 服务智能体]
        E2[专家层 - 营销智能体]
        E3[专家层 - 其他领域]
    ...
    subgraph MemoryLayer["记忆与上下文层 / Memory Layer"]
        M1[短期记忆 Redis]
        M2[中期记忆 PostgreSQL]
        M3[长期记忆 VectorDB]
        M4[上下文管理器]
    ...
    subgraph ToolLayer["工具集层 / Tool Layer"]
        T1[内容管理工具]
        T2[用户管理工具]
        T3[业务工具]
        T4[外部 API 工具]
    ...
    subgraph QualityLayer["质量与反馈层 / Quality & Feedback"]
        Q1[自动审核]
        Q2[人工审核]
        Q3[用户反馈]
        Q4[效果评估]
    ...
    GatewayLayer --> SchedulerLayer
    SchedulerLayer --> AgentLayer
    AgentLayer --> MemoryLayer
    AgentLayer --> ToolLayer
    AgentLayer --> QualityLayer
    QualityLayer --> MemoryLayer
    ...
    style QualityLayer fill:#e0f2f1
```
